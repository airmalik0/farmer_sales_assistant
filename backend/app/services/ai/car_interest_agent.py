...
- `brand`: Марка (массив строк). Нормализуй: "бмв" -> "BMW".
- `model`: Модель (массив строк). Нормализуй: "х5" -> "X5".
...
- `exterior_color`, `interior_color`: Цвет (массив строк).
- `engine_type`: тип двигателя "gas", "diesel", "hybrid", "electric".
- `drive_type`: привод "AWD", "FWD", "RWD".
- `notes`: Короткий комментарий, если есть важная деталь, не укладывающаяся в поля (например: "только с панорамой").
